...
import { apiSupplierSearch, apiPayables, apiCash } from '@/lib/api'
...
export default function PayablesPage() {
  ...
  const [form, setForm] = useState<{ supplier_id?: string | null; description: string; due_date: string; amount: number; notes?: string | null }>({ description: '', due_date: '', amount: 0 })
  ...
  const columns: IColumns<any>[] = [
    ...
    { label: 'Fornecedor', field: 'supplier_name', sortable: false },
    { label: 'Descrição', field: 'description', sortable: true },
    { label: 'Valor', field: 'amount', sortable: true, format: (v) => formatBRL(Number(v)||0) },
    { label: 'Pago', field: 'paid_total', sortable: true, format: (v) => formatBRL(Number(v)||0) },
  ...
  async function fetchRows() {
    try {
  const { data, count } = await apiPayables.listPaginated({ page: pagination.currentPage - 1, pageSize: pagination.itemsPerPage, sortBy: pagination.sortField, sortDir: pagination.sortOrder === 'ASC' ? 'asc' : 'desc', startDate: filters.startDate, endDate: filters.endDate, status: filters.status })
      setRows(data ?? [])
    ...
    } catch (e: any) { toast.error(e?.message ?? 'Falha ao carregar pagáveis') }
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      await apiPayables.create(form)
      toast.success('Pagável criado')
      setOpen(false)
      setForm({ description: '', due_date: '', amount: 0 })
      void fetchRows()
    } catch (e: any) { toast.error(e?.message ?? 'Falha ao criar pagável') } finally { setLoading(false) }
  }

  function openPay(row: any) { setSelectedRow(row); setPayOpen(true) }
  async function submitPay(data: { amount: number; method: 'CASH' | 'CARD' | 'PIX' | 'TRANSFER'; notes?: string | null }) {
    const session = await (async () => { try { return await apiCash.getOpenSession() } catch { return null } })()
    await apiPayables.addPayment({ payable_id: selectedRow!.id, amount: data.amount, method: data.method, cash_session_id: session?.id ?? null, notes: data.notes ?? null })
    toast.success('Pagamento registrado')
    setSelectedRow(null)
    void fetchRows()
  }

  const actions = useMemo(() => ({ custom: [ { key: 'pay', label: 'Pagar', onClick: openPay } ] }), [])
      ...
      <CustomForm open={open} onOpenChange={setOpen} title="Novo pagável" onSubmit={submit} submitLabel={loading ? 'Salvando...' : 'Salvar'} submitDisabled={loading}>
        <>
          <CustomSelect name="supplier" label="Fornecedor" value={form.supplier_id ?? ''} onChange={(v) => change('supplier_id', v)} options={options} searchable onSearch={async (q) => {
            const data = await apiSupplierSearch.search(q)
            setOptions(data.map((s: any) => ({ value: s.id, label: `${s.name ?? ''} ${s.phone ? '· ' + s.phone : ''}` })))
          }} />
  ...
  <ReceivePayModal open={payOpen} onOpenChange={setPayOpen} title="Pagar" defaultAmount={selectedRow?.remaining ?? 0} onSubmit={async (d) => { try { await submitPay(d) } catch (e: any) { toast.error(e?.message ?? 'Falha ao pagar') } }} />
    </section>
  )
}
